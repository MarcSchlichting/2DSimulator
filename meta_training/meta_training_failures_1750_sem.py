...
def evaluate_scenarios_failure_only(scenarios:list,num_per_scenario:int,hf_simulation_configuration:dict,cf_simulation_configuration):
    ...
    for s in tqdm(scenarios):
        ...
        while counter_per_scenario < num_per_scenario:
            ...
            if np.logical_or(collision_hf,collision_cf):
...
def objective(cf_simulation_config):
    trajectories, scenario_configurations, collisions = evaluate_scenarios_failure_only(scenarios,rollouts_per_scenario,hf_simulation_config,cf_simulation_config)
...
if __name__=="__main__":
    ...
    for i in range(num_iterations):
        ...
        ax_client.generation_strategy.trials_as_df.to_csv("meta_training_failures_1750_sem.csv")
        try:
            best_parameters, values = ax_client.get_best_parameters()
            best_parameter_list.append(best_parameters)
            pd.DataFrame.from_records(best_parameter_list).to_csv("meta_training_failures_1750_sem_best_parameters.csv")
