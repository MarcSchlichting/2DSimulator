...
def plot_two_scenarios(results1, results2):
    ...
    # matplotlib.use('pgf')
    plt.plot(run1_t[:run1_collision_first_idx+1],run1_x[:run1_collision_first_idx+1],"--",label="dt=0.1",color=(0,0,0))
    plt.plot(run1_t[run1_collision_first_idx:],run1_x[run1_collision_first_idx:],"--",color=(1,0,0))
    plt.plot(run2_t[:run2_collision_first_idx+1],run2_x[:run2_collision_first_idx+1],":",label="dt=0.5",color=(0,0,0))
    plt.plot(run2_t[run2_collision_first_idx:],run2_x[run2_collision_first_idx:],":",color=(1,0,0))
    plt.xlabel(r"$t$ [s]")
    plt.ylabel(r"$x$ [m]")
    plt.title("Comparison Between Two Runs")
    plt.legend()
    plt.grid(which="both")
    plt.tight_layout()
    plt.show()
