...
class StoppingCarScenario(object):
    ...
    def inner_loop_mc(self,simulation_configuration,num_trials):
        ...
        for i in range(num_trials):
            ...
            print(f"Run {i+1}/{num_trials} - Collision found: {failure}")
            if failure:
                failures.append(result)
                failure_configs.append(sampled_scenario_config)
            else:
                non_failure_configs.append(sampled_scenario_config)
...
if __name__=="__main__":
    ...
    failures, failure_configs, non_failure_configs = scenario.inner_loop_mc({"dt":0.1,"integration_method":"RK4","sensor_std":0.0},10)
