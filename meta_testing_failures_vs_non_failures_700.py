...
def evaluate_scenarios_failure_only_tri(scenarios:list,num_per_scenario:int,hf_simulation_configuration:dict,fc_cf_simulation_configuration:dict,nfc_cf_simulation_configuration:dict):
    trajectories = []   #list of tuples (hf_trajectory,cf_trajectory)
    scenario_configurations = [] #list of scenario configurations that lead to a failure
    collisions = []     #list of tuples (collision_hf, collision_cf)
    ...
    for s in tqdm(scenarios):
        ...
        while counter_per_scenario < num_per_scenario:
            ...
            results_cf_nfc = s.run(phi_sample,nfc_cf_simulation_configuration)
            collision_hf = np.any(results_hf["collision"])
            collision_cf_fc = np.any(results_cf_fc["collision"])
            collision_cf_nfc = np.any(results_cf_nfc["collision"])
            if collision_hf:
                trajectories.append((results_hf,results_cf_fc,results_cf_nfc))
                scenario_configurations.append(phi_sample)
                collisions.append((collision_hf,collision_cf_fc,collision_cf_nfc))
...
def calculate_confusion_matrix(collisions):
    ...
    TP_NFC = np.sum(np.logical_and(collisions[:,0]==collisions[:,2], collisions[:,0]==True))
    TN_NFC = np.sum(np.logical_and(collisions[:,0]==collisions[:,2], collisions[:,0]==False))
    FP_NFC = np.sum(np.logical_and(collisions[:,0]!=collisions[:,2], collisions[:,0]==False))
    FN_NFC = np.sum(np.logical_and(collisions[:,0]!=collisions[:,2], collisions[:,0]==True))

    return TP_FC,TN_FC,FP_FC,FN_FC,TP_NFC,TN_NFC,FP_NFC,FN_NFC
...
if __name__=="__main__":
    ...
    non_failure_configs = [{"dt":0.21631632,"integration_method":"RK2","sensor_std":0.24864865}]

    failure_configs = [{"dt":0.24574575,"integration_method":"RK4","sensor_std":0.34774775}]
    
    budget = [700]


    for b,nfc,fc in zip(budget,non_failure_configs,failure_configs):
        trajectories, scenario_configurations, collisions = evaluate_scenarios_failure_only_tri(scenarios,rollouts_per_scenario,hf_simulation_config,fc,nfc)
        TP_FC,TN_FC,FP_FC,FN_FC,TP_NFC,TN_NFC,FP_NFC,FN_NFC = calculate_confusion_matrix(collisions)
        miss_rate_fc = FN_FC/(TP_FC+FN_FC)
        miss_rate_nfc = FN_NFC/(TP_NFC+FN_NFC)
        # false_positive_rate= FP/(FP+TN)
        ...
        mse_list_nfc,bce_list_nfc = batch_compare_trajectories([(t[0],t[2]) for t in trajectories])
        mse_mean_nfc = torch.mean(torch.Tensor(mse_list_nfc)).item()
        mse_std_nfc = torch.std(torch.Tensor(mse_list_nfc)).item()

        print("Results for ",budget)
        print("Mean MSE FC: ",mse_mean_fc)
        print("Std MSE FC: ",mse_std_fc)
        print("Miss Rate FC: ",miss_rate_fc)
        print("Mean MSE NFC: ",mse_mean_nfc)
        print("Std MSE NFC: ",mse_std_nfc)
        print("Miss Rate NFC: ",miss_rate_nfc)
        print("stop")
